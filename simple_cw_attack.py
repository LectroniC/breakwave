...
class Attack:
    def __init__(self, sess, loss_fn, phrase_length, max_audio_len,
                 learning_rate=100, num_iterations=1000, batch_size=1,
        ...
        with open(args.summary_csv,'a') as file:
            ...
            for i in range(len(args.input)):
                if args.out is not None:
                    ...
                    path = args.outprefix+str(i)+".wav"
                print(str(i)+':')
                source_db = 20*np.log10(np.max(np.abs(audios[i])))
                final_db = 20*np.log10(np.max(np.abs(final_adv_audios[i])))
                diff_db =  final_db - source_db
                print('source dB', source_db)
                print('final dB', final_db)

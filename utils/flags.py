...
def create_flags():
    ...
    tf.app.flags.DEFINE_boolean ('randomzied_smoothing', False,   'whether to use randomized smoothing')
    tf.app.flags.DEFINE_integer ('sample_num',       1000,        'number of samples to use during randomized smoothing')
    tf.app.flags.DEFINE_float   ('smooth_sigma',     0.01,        'sigma to use during randomzied smoothing')
    tf.app.flags.DEFINE_string  ('smooth_type',      'mean',      'type of randomized smoothing: mean, median, mode')
